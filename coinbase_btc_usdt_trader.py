...
def sellBTC(qty, limit=9999):
    ...
    s = cb_client.sell_limit('USDT-BTC', qty, limit)
    if s['success']:
...
def buyBTC(qty, limit=1):
    ...
    b = cb_client.buy_limit('USDT-BTC', qty, limit)
    if b['success']:
...
def cancelAllOrders():
    orders = cb_client.get_open_orders()
...
def getOrderStatus(uuid):
    return(cb_client.get_order(uuid))
        
def getPrices(market):
    try:
        p = cb_client.get_ticker(market)
    except:
        time.sleep(5)    
        p = cb_client.get_ticker(market)
    if p['success']:
...
def getBalances():
    b = cb_client.get_accounts()
    s = dict()
    for i in b['data']:
        s[i['currency']] = i['balance']['amount']
    return s
...
out = out.append(pd.DataFrame([[
                cb_client.get_marketsummary('USDT-BTC')['result'][0]['TimeStamp'],
                np.nan,
                np.nan,
                np.nan,
                getTotalWorth(),
...
while 1:
    newtime = cb_client.get_marketsummary('USDT-BTC')['result'][0]['TimeStamp']
    b = getBalances()
    usdbal = b['USDT']
    btcbal = b['BTC']
